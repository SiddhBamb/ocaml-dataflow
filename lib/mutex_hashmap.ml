module ConcurrentHashMap = struct
  ...
  let create () =
    { table = Hashtbl.create 16; size = 0; capacity = 16; rwlock = Rw_lock.create () }
  ;;
      ...
      (fun key bucket ->
         ...
         List.iter
           (fun value ->
              ...
              let b =
                try Hashtbl.find new_table idx with
                | Not_found ->
  ...
  let insert map key value =
    ...
    if float_of_int map.size /. float_of_int map.capacity > 0.75
    then (
