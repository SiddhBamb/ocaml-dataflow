...
let accumulate_same_type_generic_inputs (inputs : Utils.generic_type list)
  : Utils.generic_type
  =
  match inputs with
  | [] -> Utils.NoOutput (* Sensible default for empty input *)
  | Utils.StringList _ :: _ ->
    let all_string_lists =
      List.filter_map
        (function
          | Utils.StringList sl -> Some sl
          | _ -> None)
        inputs
    in
  ...
  | Utils.StringIntList _ :: _ ->
    let all_si_lists =
      List.filter_map
        (function
          | Utils.StringIntList sil -> Some sil
          | _ -> None)
        inputs
    in
    Utils.StringIntList (List.concat all_si_lists)
  | Utils.StringIntListList _ :: _ ->
    let all_si_list_lists =
      List.filter_map
        (function
          | Utils.StringIntListList sil -> Some sil
          | _ -> None)
        inputs
    in
...
let get_option_exn field_name opt =
  ...
  | None ->
    failwith
      (Printf.sprintf
         "Configuration error: Missing required field '%s' in node config"
         field_name)
;;
...
module Dataflow = struct
  let run (graph_json_path : string) (functions : function_registry) =
    ...
    let sorted_nodes = Utils.topological_sort graph in
    (* list of nodes *)
    let global_output_map = Utils.create_output_map () in
    (* map from node id to output *)
    let execute_node (node : Utils.node) =
      match node.ntype with
      | Utils.FileIO ->
        let mode = get_option_exn "mode" node.config.mode in
        let path = get_option_exn "path" node.config.path in
        (* Get path for both read/write *)
        if mode = "read"
        then (
