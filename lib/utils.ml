...
let print_graph graph =
  ...
  List.iter
    ...
    graph.nodes;
  print_endline "Edges:";
  List.iter (fun edge -> Printf.printf "  %s -> %s\n" edge.from_id edge.to_id) graph.edges
;;
...
let write_output_file (path : string) (inputs : generic_type list) =
  ...
  List.iter
    (fun input ->
       match input with
       | StringList str_list -> List.iter (fun str -> output_string ch str) str_list
       | IntList int_list ->
       ...
       | StringIntList str_int_list ->
         List.iter
           (fun (str, int) -> output_string ch (str ^ " " ^ string_of_int int))
           str_int_list
       | StringIntListList str_int_list_list ->
         List.iter
           (fun (str, int_list) ->
              List.iter
                (fun int -> output_string ch (str ^ " " ^ string_of_int int))
                int_list)
           str_int_list_list
