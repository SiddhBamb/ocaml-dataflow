...
let json_config_to_node_config (json_config : Yojson.Basic.t) : Utils.node_config =
  ...
  let function_name =
    match function_name_json with
    | `Null -> None
    | _ -> Some (Yojson.Basic.Util.to_string function_name_json)
  in
  let threads =
    Yojson.Basic.Util.member "threads" json_config |> Yojson.Basic.Util.to_int
  in
  { path; mode; function_name; threads }
;;
...
let json_node_to_node (json_node : Yojson.Basic.t) : Utils.node =
  ...
  { id; ntype; config = node_config }
;;

let json_edge_to_edge (json_edge : Yojson.Basic.t) : Utils.edge =
  let from_id =
    Yojson.Basic.Util.member "from" json_edge |> Yojson.Basic.Util.to_string
  in
...
let json_to_graph (filename : string) : Utils.graph =
  ...
  let job_name =
    Yojson.Basic.Util.member "job_name" json_str |> Yojson.Basic.Util.to_string
  in
