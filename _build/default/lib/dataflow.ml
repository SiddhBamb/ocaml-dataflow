...
module MakeMapReduce (Key : Map.OrderedType) = struct
    ...
    let shuffle_task_runner (maps: 'v KMap.t list) : 'v list KMap.t =
        ...
        let merge_fun (_key : Key.t) (list_opt : 'v list option) (v_opt : 'v option) : 'v list option =
            ...
            | None, None -> None (* Should not happen in Map.merge logic, but handle defensively *)
        in
        List.fold_left (fun acc_map current_map ->
            KMap.merge merge_fun acc_map current_map
        ) init_map maps
    ;;
    ...
    let reduce_task_runner (map: 'v list KMap.t) (reduce_func: Key.t -> 'v list -> 'r) : 'r KMap.t =
    ...
    let run (input_chunks: 'a input_chunk list)
            ...
            ~(reduce_func: Key.t -> 'v list -> 'r) : 'r KMap.t =
