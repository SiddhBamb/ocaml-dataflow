let read_input file_path =
  ...
  let records =
    List.mapi (fun line_idx data_line_str ->
      match String.split_on_char ' ' data_line_str with
      | [user_str; item_str; rating_str] ->
          (try Some (user_str, (int_of_string item_str, int_of_string rating_str))
           with Failure _ -> failwith (Printf.sprintf "Invalid number format on line %d: %s" (line_idx + 2) data_line_str))
      | [] when line_idx = (List.length data_lines - 1) && data_line_str = "" ->
          None
      | _ -> failwith (Printf.sprintf "Invalid data line format on line %d: expected user item rating, got: \"%s\"" (line_idx + 2) data_line_str)
    ) data_lines
...
let normalize_ratings (input : string * ((int * int) list)) : (int * int) list =
  ...
  let average_rating = List.fold_left (fun acc (_, rating) -> acc + rating) 0 ratings / List.length ratings in
...
let compute_corating (input : (int * int) list) : ((int * int) * (int * int * int)) list =
  ...
  let similarity = List.fold_left (fun acc (item1, rating1) -> List.fold_left (fun acc (item2, rating2) -> if item1 < item2 then ((item1, item2), (rating1 * rating2, rating1 * rating1, rating2 * rating2)) :: acc else acc) acc input) [] input in
  similarity
;;

let compute_similarity (input : (int * int) * ((int * int * int) list)) : int * (int * float) =
  let ((i, j), coratings) = input in
  let total_dot_product = List.fold_left (fun acc (dot, _, _) -> acc + dot) 0 coratings in
  let total_magI = sqrt (float_of_int (List.fold_left (fun acc (_, magI, _) -> acc + magI) 0 coratings)) in
  let total_magJ = sqrt (float_of_int (List.fold_left (fun acc (_, _, magJ) -> acc + magJ) 0 coratings)) in
...
let run_matrix_factorization (num_domains : int) (file_path : string) : (int * ((int * float) list)) list =
  let records = read_input file_path in
  let grouped_by_user = Nodes.Groupby.run_groupby ~num_domains { input = records } in
  let normalized_ratings = Nodes.Computation.run_computation ~num_domains { input = grouped_by_user; transform = normalize_ratings } in
  let corating = List.concat (Nodes.Computation.run_computation ~num_domains { input = normalized_ratings; transform = compute_corating }) in
  let grouped_by_pairs = Nodes.Groupby.run_groupby ~num_domains { input = corating } in
  let similarity = Nodes.Computation.run_computation ~num_domains { input = grouped_by_pairs; transform = compute_similarity } in
  let filtered_similarity = Nodes.Filter.run_filter ~num_domains { input = similarity; pred = filter_similarity } in
  let grouped_by_item = Nodes.Groupby.run_groupby ~num_domains { input = filtered_similarity } in
  let top_k = Nodes.Computation.run_computation ~num_domains { input = grouped_by_item; transform = get_top_k } in
  top_k
;;
