...
let run_wordcount_new ~num_domains file =
  ...
  let mapped = Computation.run ~num_domains:n_dom { input = words; transform = map } in
  let grouped = GB.run ~num_domains:n_dom { input = Shards.concat mapped } in
  let reduced =
    Computation.run
      ~num_domains:n_dom
      { input = Shards.concat grouped; transform = reduce }
