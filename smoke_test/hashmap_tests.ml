...
let test_insert_delete () =
  ...
  (* Insert a list with one value *)
  ConcurrentHashMap.delete map "key";
  (* Delete the key *)
...
let test_overwrite_key () =
  ...
  ConcurrentHashMap.insert map "dup" 2;
  (* Insert new value *)
...
let test_concurrent_inserts () =
  ...
  if
    ...
    && List.for_all
         (fun key -> List.length (ConcurrentHashMap.read map key) = 1000)
         all_keys
  then print_endline "PASS test_concurrent_inserts passed"
...
let test_speedup () =
  ...
  let concurrent_time, _ =
    time (fun () ->
      Domainslib.Task.run pool (fun _ ->
        let tasks =
          List.init num_tasks (fun i ->
            Domainslib.Task.async pool (fun () ->
  ...
  let concurrent_with_regular_map_time, _ =
    time (fun () ->
      Domainslib.Task.run pool (fun _ ->
        let tasks =
          List.init num_tasks (fun i ->
  ...
  Printf.printf
    "Regular Map insert time (using threads): %.4f seconds\n"
    concurrent_with_regular_map_time;
  Printf.printf "Regular Map insert time: %.4f seconds\n" regular_time;
  if concurrent_time < regular_time
  then
    Printf.printf
      "PASS ConcurrentHashMap was faster by %.2fx\n"
